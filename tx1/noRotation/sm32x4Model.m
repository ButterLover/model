...
for w=1:rxN
    ...
    ant_rx=psht(array_rx, dir_rx(:,:,w), fc, false); % 200x2
    ant_rx=reshape(ant_rx.', elem_rx, 1, rayN); % 2x1x200
    % Applying antenna pattern on plane wave
    tx=repmat(ant_tx, elem_rx, 1, 1); % 2x32x200
    rx=repmat(ant_rx, 1, elem_tx, 1); % 2x32x200
    ant=reshape(tx.*rx, [],rayN); % 64x200
    am=bsxfun(@times, ant.', al_lin(:,:,w)); % 200x64 200x1
    % Calculating transfer function from plane wave
    hf(:,:,w)=pw2hf(am, toa(:,:,w), fc, bw, Nf);    % 801x64
    
    % Finding steering vector at rx side
    hf_t=reshape(hf(:,:,w).', elem_rx, elem_tx, []); % 2x32x801
    hf_rx=squeeze(hf_t( :, 1, :));  % 2x801 Assuming all rx element see the same from tx
    ...
    hf_sh=permute( conj( hf_s), [2 1 3] );
    R=times3d( hf_s, hf_sh);
