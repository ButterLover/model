...
%     al_rx=reshape(repmat(al_lin(:,:,w), 1, elem_rx*Nb_rx), rayN, elem_rx, Nb_rx);
    
%     as_rx=steerVec( array_rx, al_lin( :, :, w), dir_rx(:,:,w), fc, 1);
%     ant_rx=reshape(ant_rx.', elem_rx, Nb_rx, rayN); % 8x1x200
%     ant_rx=squeeze(sum(ant_rx,2)); % 4x200
    ...
    % Applying steering vector on plane wave
    ant_rx=reshape( ant_rx, elem_rx, Nb_rx, rayN);
    ants_rx=squeeze(sum( repmat( as_rx, 1, 1, rayN).*ant_rx, 1)); % 4x200
    ants_rx=reshape( ants_rx, Nb_rx, 1, rayN);
    ants=repmat(ant_tx, Nb_rx, 1, 1).*repmat(ants_rx, 1, elem_tx, 1);
    ants=reshape(ants, Nb_rx*elem_tx, rayN);
    as=bsxfun(@times, ants.', al_lin(:,:,w)); % 200x128 200x1
    hfs(:,:,w)=pw2hf(as, toa(:,:,w), fc, bw, Nf);    % 801x128
    hf_s=reshape(hfs(:,:,w).', Nb_rx, elem_tx, Nf); % 4x32x801
